...
- **`World.java` decomposition (mirror of `InfdevProject/Main` reference architecture)**: staged extraction of the monolithic 3426-line `World.java` into collaborator classes, applied identically to client and server, one stage at a time, compile + parity-check after each:
  ...
  - **Stage 3 PENDING — `BlockTickScheduler`**: `scheduledTickTreeSet`/`scheduledTickSet` + `scheduleBlockUpdate`/`TickUpdates`. Beware `WorldClient` overrides both.
  - **Stage 4 PENDING — lighting**: extract only non-Starlight lighting bookkeeping in use, remove dead old-lighting leftovers.
  - **Stage 5 PENDING — `AtmosphereCalculator`**: static sky/fog/cloud/star color + brightness helpers, preserving `Seasons`/`colouredAthmospherics` overrides.
  - **Stage 6 PENDING — `Explosion`**: keep `WorldServer` `newExplosion`/`newBlockExplosion` factory hooks.
...
## Next Move
1. Continue `World.java` decomposition: **Stage 3 — `BlockTickScheduler`** (scheduled tick tree/set + `scheduleBlockUpdate`/`getPendingBlockUpdate`), then Stages 4-6. (Stages 1-2 complete; rendering bug fix pending user test.)
2. Continue `net.minecraft.client.gui` package cleanup: process `GuiButton.java`, `GuiTextField.java`, `GuiSlider.java`, `GuiIngame.java`, `GuiIngameMenu.java`, `GuiMainMenu.java`, and all remaining GUI files.
