...
## Next Move
1. Continue `World.java` decomposition: **Stage 4 — lighting bookkeeping** (extract only non-Starlight lighting in use, remove dead old-lighting leftovers), then Stages 5-6 (`AtmosphereCalculator`, `Explosion`). (Stages 1-3 complete; SaveConverterMcRegion constructor fix pending user test of block ticks in game.)
