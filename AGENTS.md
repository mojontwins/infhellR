...
## Next Move
1. Continue `World.java` decomposition: **Stage 5 — `AtmosphereCalculator`** (static sky/fog/cloud/star color + brightness helpers, preserving `Seasons`/`colouredAthmospherics` overrides), then Stage 6 (`Explosion`). (Stages 1-4 complete; SaveConverterMcRegion constructor fix pending user test of block ticks in game.)
