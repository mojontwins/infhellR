...
## Work State
### Completed
- **Open issue — riding a pig in first person, the pig's head flickers/vanishes before the screen border when looking up** (client-only, `EntityRenderer.java`): first candidate root cause was the view-space eye-height translate — `orientCamera` applies `glTranslatef(0, eyeHeight=-1.62, 0)` AFTER the pitch/yaw rotation, so pitching up makes the eye rise `posY+1.62(1-cosθ)` and slide rearward `1.62·sinθ`, shoving the pig's head into the near plane (0.05). A fix was implemented (apply the eyeHeight translate in WORLD space — before the rotation — and skip the after-rotation one when riding the pig in first person; verified numerically, client compiled EXIT 0) but the user's in-game test showed **no visible change** — behaviour identical, so the changes were **reverted** (file is byte-identical to HEAD). NOTE FOR LATER: the pitched-eye-over-run is NOT the (whole) cause; re-investigate from scratch — next candidates after ruling out near-plane clipping of on-screen geometry (3 numeric sweeps agree nothing visible near-clips at the ANCHORED eye), whole-entity frustum cull (camera inside the pig bbox → always passes), z-fighting (saddle/pig head gap 0.03 well above depth precision), and head-roll animation: camera is embedded in the head/body volume (mount 0.675 inside body 0.445–1.445 and head 0.508–1.008), so suspect a per-frame pass/fail at a surface near the eye as the pig animates/bobs.
- **Pig steering feature complete**: both trees byte-identical `EntityPig.java` + `Packet27Position.java` (steering branch in `updateEntityActionState`, `speedModifier=2.0`, `TURN_RATE=30°/tick`, clamp move input, `Packet27Position` setters, `EntityClientPlayerMP.sendPigSteeringInput` each tick); client + server compile EXIT 0.
- **HauntedCow curse now snaps SMP clients to night instantly** (`WorldServer.java`, server-only): the shared `EntityCow` curse already calls `worldObj.setWorldTime((worldTime/24000)*24000+18000)` when transforming, but the dedicated server only broadcast `Packet4UpdateTime` on the periodic one-second sync, so clients never saw the instant night. Added a `setWorldTime(long)` override in `WorldServer` that calls `super.setWorldTime` then `configManager.sendPacketToAllPlayersInDimension(new Packet4UpdateTime(time), worldProvider.worldType)` (null-guards `mcServer`/`configManager` for constructor-time safety). Bonus: `/time`, `/day`, and scheduled-tick time shifts also broadcast automatically. Server-only; parity 830 identical / 37 substantive unchanged; client + server compile ZERO errors; `build.bat` exports both jars.
- **Bug fix — server crash on respawn "already is in chunk 263, 6"** (server-only): `Entity.equals` compares by `entityId` (server `Entity.java:112`); `ServerConfigurationManager.recreatePlayerEntity` builds a fresh `EntityPlayerMP` and copies `oldPlayer.entityId` onto it, so any stale chunk subscription that the square-based `removePlayer` (centred on `managedPos`) never reaped made `List.contains(newPlayer)` match on the same id → throw at `PlayerManager.addPlayer`. Fixes: added `PlayerHash.getAllValues()` snapshot iterator; `PlayerManager.removePlayer` now sweeps ALL chunk instances after the square-based loop; `recreatePlayerEntity` gained an explicit world-spawn fallback when `spawnCoordinates == null` (user directive — valid crying-obsidian spawn wins, else `worldServer.getSpawnPoint()` with ±10 jitter via local `java.util.Random()` since `Entity.rand` is `protected`, `!hasNoSky` guard, Y via `findTopSolidBlock`, then `Packet70Bed(0)`).
- **Bug fix — server crash during movement "already is in chunk 256, -12"** (`PlayerManager.java`, server-only): commit `e4a5b02` rewrote the misnamed `isOutsidePlayerViewRadius` (old: returned "inside the box", callers negated with `!`) into correctly-semantic `isOutsideViewRadius` (returns true when outside), but `updateMountedMovingPlayer`'s two callers kept the `!` → both conditions inverted, so players never migrated between view-radius chunk lists and the origin-shift re-add always misfired. Fix: dropped the `!` at both call sites. User-verified mid-game.
- **Bug fix — server startup NPE (graveyard flower drop during spawn probe)** (`WorldServer.java`, server-only): `World.<init>` runs `getInitialSpawnLocation()` (spawn probe) inside the *superclass* constructor, before `WorldServer`'s field initializer `entityRemoval = new MCHash()` had run. Probing generated+populated a graveyard chunk whose `BuildingGraveyard.populate` → `Building.addSpawner` → `setBlockWithNotify` → neighbor change made a `BlockFlower` lose support → `dropBlockAsItem` → `spawnEntityInWorld` → `WorldServer.obtainEntitySkin` dereferenced the still-null hash → NPE on first server start. Fix: `entityRemoval` is now lazily created in `obtainEntitySkin`/constructor body (no inline initializer, so nothing is clobbered), `releaseEntitySkin` guards null, `getEntityByID` returns null while uninitialised. Client `WorldClient` override only forwards to world-accesses (no twin issue); parity unaffected. Server compiles 0 errors.
- **Height-only chunk queries — caching + exact density-only height gen DONE** (`docs/height_query_caching.md`, both trees): new `net.minecraft.game.world.TerrainHeightQueryCache` (access-order LRU, cap 256, entry = `byte[256]` height map + `isOcean` + `isUrbanChunk`); `World`'s four height facades (`getLandSurfaceHeightValue`/`isOceanChunk`/`isUrbanChunk`/`justGenerateForHeight`) short-circuit live chunks → cache (hit synthesises `new Chunk(world,cx,cz)`); `World.evictHeightQuery` wired into `ChunkProvider.unload100OldestChunks` (both trees), `ChunkProviderServer.unload100OldestChunks` (server), `ChunkProviderClient.unloadChunk` (client). **2-A**: `justGenerateForHeight` is now density-only (no block arrays) for all three providers — overworld lattice 5×17×5, sky 3×33×3, hell blank; semantics equal `generateTerrain`'s height field (`topY` = topmost `density>0`, `isOcean` folds at sea level). **2-C**: `ChunkProviderGenerate.initializeNoiseField` biome window batch — the 625×1×1 `getBiomeGenAt`+`isUrbanChunk` collapsed into one 21×21 `loadBlockGeneratorData` grid + 4 `isUrbanChunk` coords; bit-exact `invalidateCity` handling (per-cell special-override re-mask via `getBiomeFromLookup` comparison, tail flag reinstated from the (20,20) corner cell). Probe-verified byte-equal (`VERIFY PASS`, 180 chunks), cache tests + live-chunk precedence pass; client+server compile 0 errors; parity 830 identical/37 substantive unchanged; `build.bat` exports both jars. Doc §3.3/§6 has the full implementation log.
...
### Completed
- **`ensureSchematic` rebuild mechanic REMOVED** (user directive — "remove the `ensureSchematic` methods completely; this mechanic is not needed", both trees, byte-identical): a dynamic-schematic feature is now released by *forgetting it* — `FeatureDynamicSchematic.onChunkPopulated` frees `schematic`, clears `specialBlocks`, AND removes itself from the provider (`FeatureProvider.removeFeature(Feature)`, identity-guarded: only if `featureList.get(originHash) == feature`) when `pendingPopulates` hits 0. Soundness invariant: a feature is in `featureList` iff it is still drawing, so a freed schematic can never be drawn again; a later gate re-run over a fully-built box is short-circuited by the retained `isFeatureFullyBuilt` guard and the cell memoizes in `noFeatureChunks`. Removed: `Feature.ensureSchematic()` no-op, `FeatureDynamicSchematic.ensureSchematic()` override, the two `schematic == null → ensureSchematic()` safety nets in `drawPieceOnPopulation`/`drawSpecialBlocksForChunk`, the pre-draw `feature.ensureSchematic()` call in `getNearestFeatures`, `FeatureIcePalace.ensureSchematic` (sky rebuild gate), and the dead `totalPopulates` field (`buildSchematic` arms `pendingPopulates` directly via `computePopulatesRemaining`). Temp probe counters (`pfCalls`/`pfReHits`/`pfRegenPopulates` + failsafe stack print) also reverted from `FeatureProvider.populateFeatures` after verification. Verification (all against a clean `build_test_es` rebuild): FeatureProbe golden regenerated (`feature_golden_removal.txt`, 14 lines — the pre-clean earlier baseline was stale) → `VERIFY PASS` ×3 + `RELEASE CHECK PASS` (5 resident dynamic-schematic features all live, pending>0, schematic allocated); FeatureRejoinProbe (census from setup()-prints, block→chunk conversion `(center-8)>>4`) → `PASS=1 FAIL=0` (interior fossil built once, box-final → not re-created, featureList after re-entry holds zero fully-final entries). Client + server compile ZERO errors; parity 830 identical / 37 substantive unchanged; `build.bat` `BUILD_EXIT=0` both jars exported. Doc §9 added (`docs/feature_provider_optimizations_postmortem.md`), §3.3 marked superseded.
...
- **Bug fix — McRegion worlds "missing block data", builds reverted on reload**: singleplayer saves are McRegion (`.mcr`) and load via `McRegionChunkLoader.loadChunk`, which still gated solely on `hasKey("Blocks")` — new-format chunks store blocks as `SubchunkMask` + parallel section lists (`Height=SECTION_HEIGHT`, no `Blocks` tag), so every saved chunk was rejected, regenerated fresh, and prints "Chunk file at x,z is missing block data, skipping". Fixed the gate to also accept `Height == Chunk.SECTION_HEIGHT` (mirrors `ChunkLoader.loadChunk`); saved files keep their data, so untouched by relight. Applied to both trees, byte-identical; client + server compile ZERO errors.
- **Bug fix — MapGenCity NPE `raiseTerrain` when a city cross-road modified an already-generated neighbor**: `markOrProcessChunk` passed `chunk.blocks`/`chunk.data` (flat generation buffers, now null after `loadFlatBlocks` slices them in `provideChunk`). Fix: new `Chunk.exportFlatBlocks128`/`exportFlatData128`/`importFlatBlocks128` (flat 128-high staging round-trip through subchunk storage, re-materializes sections gaining blocks, refreshes empty flags, does NOT relight — matches historical direct-array writes); `markOrProcessChunk` now stages through them. Both trees byte-identical; client + server compile ZERO errors.
- **Bug fix — generation-stage light fully bright (sea under highway / underwater terrain scanner-bright)**: stored sky light was **15 everywhere in freshly generated chunks** — y=40 deep in solid stone, under the highway deck, underwater. Root cause: `Chunk.ensureSubchunk` pre-filled every materialized sky plane to 15 (deviating from vanilla, whose `ExtendedBlockStorage` starts skylight at **0**). Both `Chunk.generateSkylightMap` (vanilla top-down gradient, `Chunk.java:418`) and Starlight's `initSkylight`/`initBlockLight` are **increase-only** (`performLightIncrease`; neither does a decrease pass during init), so a pre-filled 15 under any opaque block (roof/terrain/water) could never be lowered — exactly why "generation blocks" were wrong-lit while "population blocks" (incremental `setBlockIDWithMetadata` → `updateLight` → `checkSkyEmittance`, which does run decreases) shadowed correctly. **Proven headlessly** via `C:\Users\na_th\AppData\Local\Temp\opencode\lightprobe\LightProbe.java` (runs the real `SaveConverterMcRegion`→`World`→`ChunkProviderGenerate`→Starlight pipeline drumming block+sky for highway/ocean/land columns and a simulated pop-stage roof): pre-fix all sky=15 incl. stone at y=40; post-fix 15 above the deck/ground and **0 for the deck block and everything below it**, incremental roof test unchanged (13 under a 3-block gap, 0 in terrain) — stable across reruns. Fix (both trees, byte-identical): (1) `ensureSubchunk` no longer pre-fills sky (new `NibbleArray` is already 0; javadoc now documents the null-subchunk=implicit-15 invariant); (2) `clearAllLights` now zeroes **both** light planes (was sky-only — the server copy still had the old `setAll(15)`, a pre-existing client/server divergence now normalized); (3) `initLightingForRealNotJustHeightmap` calls `clearAllLights()` first so every relight is a complete, self-contained rebuild from a zeroed slate (repeatable for secrets: `SchematicsBigShip:127`, migration); (4) `ChunkLoader` stamps **`LightVersion=2`** and the load gate now reads the **numeric** version (`getByte >= 2`) instead of `hasKey` — v1 files/profiles from the pre-fill build (and any untagged) relight exactly once on load, then re-save as v2. Client + server compile ZERO errors; parity 827 identical / 38 substantive (was 826/39 — `Chunk.java` itself dropped out of the diff list).

- **BlockFire de-hardcoding DONE** (`435b480`, both trees, committed+pushed): fire spread rates moved out of `BlockFire`'s two `int[256]` arrays into per-block virtual methods `Block.getEncouragementToFire()`/`getAbilityToCatchFire()` (defaults `0,0`). New `BlockPlanks` (5,20); overrides on `BlockLog` (5,5), `BlockHollowLog` (5,5), `BlockLeaves` (30,60), `BlockBookshelf` (30,20), `BlockTNT` (15,100), `BlockTallGrass` (60,100), `BlockCloth` (30,60), `BlockFence` (5,20 only when `Material.wood`, via material check so the iron fence stays 0), `BlockBone` explicitly 0 (extends `BlockLog` but must not burn); `BlockStairs` delegates to its `modelBlock` (wood stairs burn, stone don't). `BlockFire` lost `setBurnRate()`+`initializeBlock()` and the dead commented beta-1.6 block; `tryToCatchBlockOnFire`/`canBlockCatchFire`/`getChanceToEncourageFire` now dispatch via `Block.blocksList` with null guard; obfuscated locals renamed. Public `canBlockCatchFire(IBlockAccess,...)` signature preserved (`RenderBlockFire` caller). Client+server compile ZERO errors; parity 824 identical / 42 substantive (the 2 touched-file diffs — BlockStairs texture-metadata arg, BlockTallGrass trailing line — pre-existed at HEAD). Same commit also carried unrelated in-tree edits: GUI slider fix, "InfHell" brand, axe/pickaxe stair metadata entries, BlockDoor step-support, World city-chance flip, `docs/height_query_caching.md`.
...
## Next Move
0. **Server playtesting fixes complete** (PlayerManager movement crash, respawn crash, HauntedCow instant-night broadcast) — pending user verification in SMP (transform a cow with a HauntedCow to confirm clients snap to night). If confirmed, proceed with GUI cleanup.
1. **`ensureSchematic` removal + temp counter revert COMPLETE** (probes re-verified against a clean build, jars exported) — the only open thread is the still-unexplained 3–4 "Fossils @" prints on a brand-new world at creation (re-entry theory disproven by rejoin probe: `PASS=1 FAIL=0`). If still in scope, trace creation-time flow (`Minecraft.java:1922 new World` → `World.chunkProvider` field init line 174 / `getChunkProvider()` line 307 / `getInitialSpawnLocation()` line 325 / `spawnPlayerWithLoadedChunks`); probes currently skip the spawn-probe path for determinism, so a dedicated probe for the creation path is needed. Otherwise leave as documented-wontfix.
2. Continue **`net.minecraft.client.gui` package cleanup**: process `GuiButton.java`, `GuiTextField.java`, `GuiSlider.java`, `GuiIngame.java`, `GuiIngameMenu.java`, `GuiMainMenu.java`, and all remaining GUI files (256-height implementation and its post-implementation lighting/`LightVersion` fixes are complete — see Completed).
