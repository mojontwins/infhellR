...
public class ItemRenderer {
	...
	public ItemRenderer(Minecraft minecraft1) {
		this.mc = minecraft1;
		this.mapItemRenderer = new MapItemRenderer(minecraft1.fontRenderer, minecraft1.renderEngine);
	}
	...
	public void renderItem(EntityLiving entityLiving1, ItemStack itemStack2) {
		...
		if(itemStack2.itemID < 256 && RenderBlocks.renderItemIn3d(Block.blocksList[itemStack2.itemID].getRenderType())) {
			...
			TextureAtlas itemAtlas = itemStack2.itemID < 256 ? TextureAtlas.TERRAIN : TextureAtlas.ITEMS;
			AtlasTexel.calc(i4, itemAtlas);
			float f5 = TexelScale.u(itemAtlas, (float)AtlasTexel.u);
			float f6 = TexelScale.u(itemAtlas, (float)AtlasTexel.u + itemAtlas.tileSpan);
			float f7 = TexelScale.v(itemAtlas, (float)AtlasTexel.v);
			float f8 = TexelScale.v(itemAtlas, (float)AtlasTexel.v + itemAtlas.tileSpan);
			float f9 = 1.0F;
			float f10 = 0.0F;
			float f11 = 0.3F;
			...
			for(i14 = 0; i14 < 16; ++i14) {
				f15 = (float)i14 / 16.0F;
				f16 = f6 + (f5 - f6) * f15 - 0.5F / (float) itemAtlas.width;
				f17 = f9 * f15;
			...
			for(i14 = 0; i14 < 16; ++i14) {
				f15 = (float)i14 / 16.0F;
				f16 = f8 + (f7 - f8) * f15 - 0.5F / (float) itemAtlas.height;
				f17 = f9 * f15 + 0.0625F;
	...
	public void renderItemInFirstPerson(float f1) {
		...
		float f6;
		float f7;
		if(entityPlayerSP3 instanceof EntityPlayerSP && GameSettingsValues.retardedArm) {
	...
	private void renderInsideOfBlock(float f1, int i2) {
		...
		float f8 = 1.0F;
		float f9 = -0.5F;
		float f10 = 2.0F * TextureAtlas.TERRAIN.pixelU;
		float f11 = TexelScale.u(TextureAtlas.TERRAIN, (float)(i2 % 16)) - f10;
		float f12 = TexelScale.u(TextureAtlas.TERRAIN, (float)(i2 % 16) + (float) TextureAtlas.TERRAIN.tileSpan) + f10;
		float f13 = TexelScale.v(TextureAtlas.TERRAIN, (float)(i2 / 16)) - f10;
		float f14 = TexelScale.v(TextureAtlas.TERRAIN, (float)(i2 / 16) + (float) TextureAtlas.TERRAIN.tileSpan) + f10;
		tessellator3.startDrawingQuads();
	...
	private void renderFireInFirstPerson(float f1) {
		...
		for(int i4 = 0; i4 < 2; ++i4) {
			GL11.glPushMatrix();
			int i5 = Block.fire.blockIndexInTexture + i4 * TextureAtlas.TILE;
			AtlasTexel.calc(i5, TextureAtlas.TERRAIN);
			float f8 = TexelScale.u(TextureAtlas.TERRAIN, (float)AtlasTexel.u);
			float f9 = TexelScale.u(TextureAtlas.TERRAIN, (float)AtlasTexel.u + TextureAtlas.TERRAIN.tileSpan);
			float f10 = TexelScale.v(TextureAtlas.TERRAIN, (float)AtlasTexel.v);
			float f11 = TexelScale.v(TextureAtlas.TERRAIN, (float)AtlasTexel.v + TextureAtlas.TERRAIN.tileSpan);
