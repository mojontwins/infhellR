...
public class ClippingHelperImpl extends ClippingHelper {
	...
	private FloatBuffer clippingMatrixBuffer = GLAllocation.createDirectFloatBuffer(16);
	...
	private void normalize(float[][] matrix, int row) {
		float length = MathHelper.sqrt_float(matrix[row][0] * matrix[row][0] + matrix[row][1] * matrix[row][1] + matrix[row][2] * matrix[row][2]);
		matrix[row][0] /= length;
		matrix[row][1] /= length;
		matrix[row][2] /= length;
		matrix[row][3] /= length;
	...
	private void init() {
		...
		this.clippingMatrixBuffer.clear();
