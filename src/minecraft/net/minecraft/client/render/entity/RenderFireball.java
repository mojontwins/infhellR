...
public class RenderFireball extends Render {
	public void doRenderFireball(EntityFireball fireball, double x, double y, double z, float yaw, float partialTicks) {
		GL11.glPushMatrix();
		GL11.glTranslatef((float)x, (float)y, (float)z);
		GL11.glEnable(GL12.GL_RESCALE_NORMAL);
		float scale = 2.0F;
		GL11.glScalef(scale / 1.0F, scale / 1.0F, scale / 1.0F);
		int iconIndex = Item.snowball.getIconFromDamage(0);
		this.loadTexture("/gui/items.png");
		Tessellator tessellator = Tessellator.instance;
		AtlasTexel.calc(iconIndex, TextureAtlas.ITEMS);
	...
	public void doRender(Entity entity, double x, double y, double z, float yaw, float partialTicks) {
		this.doRenderFireball((EntityFireball)entity, x, y, z, yaw, partialTicks);
	}
}
