...
public class RenderThrowablePotion extends Render {
	...
	public void doRender(Entity entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		ItemPotion itemPotion = ((EntityThrowablePotion)entity).itemPotion;
		if(itemPotion == null) return;

		int potionColor = itemPotion.getColorFromDamage(0);
		float red = (float)(potionColor >> 16 & 255) / 255.0F;
		float green = (float)(potionColor >> 8 & 255) / 255.0F;
		float blue = (float)(potionColor & 255) / 255.0F;
		float brightness = 1.0F;
		GL11.glColor4f(red * brightness, green * brightness, blue * brightness, 1.0F);

		this.loadTexture("/gui/items.png");
		Tessellator tessellator = Tessellator.instance;
		AtlasTexel.calc(itemPotion.iconIndex, TextureAtlas.ITEMS);
