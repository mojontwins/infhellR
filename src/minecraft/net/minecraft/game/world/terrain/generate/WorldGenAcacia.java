...
public class WorldGenAcacia extends WorldGenMojon {
	...
	private static final int leavesMeta = (2 | EnumTreeType.BAOBAB.getLeafMetadata());
	...
	public boolean generate(World world, Random rand, int x0, int y0, int z0) {
		...
		if(!this.validGround(world, x0, y0 - 1, z0) || y0 + height > world.getWorldHeight() - 2) return false;
		...
		for(int j = 0; j <= 1; j ++) {
			...
			for(int direction : Direction.HORZ_PLANE) {
				...
				int vz2 = vz; if(rand.nextBoolean()) vz2 += vz;
