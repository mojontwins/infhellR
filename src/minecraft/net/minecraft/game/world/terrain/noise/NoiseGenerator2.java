...
public class NoiseGenerator2 {
	...
	public void generateNoise(double[] densityArray, double x, double z, int xSize, int zSize, double xScale, double zScale, double levelScale) {
		int idx = 0;

		for(int xIt = 0; xIt < xSize; ++xIt) {
			double d16 = (x + (double)xIt) * xScale + this.field_4292_a;

			for(int zIt = 0; zIt < zSize; ++zIt) {
				double d19 = (z + (double)zIt) * zScale + this.field_4291_b;
				...
				int densityIndex = idx++;
				densityArray[densityIndex] += 70.0D * (d21 + d23 + d25) * levelScale;
			}
		}
